...
import EditAgentProfileModal from '../components/agent/EditAgentProfileModal';
import EditAgentGoalModal from '../components/agent/EditAgentGoalModal';
import EditInsurerPOCsModal from '../components/agent/EditInsurerPOCsModal';
...
const AgentProfilePage = () => {
    ...
    const fetchAgentData = useCallback(async () => {
        ...
        try {
            ...
            if (!profileRes.ok) {
                let e = `Profile Error (${profileRes.status})`;
                try {
            ...
            setAgentProfileData(profileData || null);

            if (!goalRes.ok) {
                let e = `Goal Error (${goalRes.status})`;
                try {
...
>>>>>>> ecf9462fa5ba82ff87c3090670b89d2a85b693b7
    ...
    if (!agentProfileData) {
        return <div className="text-center p-10 text-gray-500">Could not load agent profile.</div>;
    }
    console.log(agentProfileData,"agentProfileData")
    const userPart = agentProfileData
    const userEmail=agentProfileData.email
    ...
    return (
        <div style={{ '--brand-purple': themeColors.brandPurple }}>
            ...
            {error && agentProfileData && (
                <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-2 rounded relative m-4 text-sm" role="alert">
                ...
                <dl className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-4 text-sm">
                    <div className="md:col-span-1 space-y-4">
                        <div>
                            ...
                            <dd className="text-gray-800 mt-1">{userEmail || 'N/A'}</dd>
                        </div>
                        <div>
                            <dt className="font-medium text-gray-500">Mobile Number</dt>
                            <dd className="text-gray-800 mt-1">{profilePart?.mobile?.String || <span className="italic text-gray-400">Not Set</span>}</dd>
                        </div>
                        <div>
                            <dt className="font-medium text-gray-500">Gender</dt>
                            <dd className="text-gray-800 mt-1">{profilePart?.gender?.String || <span className="italic text-gray-400">Not Set</span>}</dd>
                        </div>
                        <div>
                            <dt className="font-medium text-gray-500">Profile Created</dt>
                            <dd className="text-gray-800 mt-1">{formatDate(userPart?.createdAt) || 'N/A'}</dd>
                        </div>
                        <div>
                            <dt className="font-medium text-gray-500">User Type</dt>
                            <dd className="text-gray-800 mt-1 capitalize">{userPart?.userType || 'N/A'}</dd>
                        </div>
                    </div>
                    <div className="md:col-span-2 space-y-4">
                        <div>
                            <dt className="font-medium text-gray-500">Postal Address</dt>
                            <dd className="text-gray-800 mt-1 whitespace-pre-line">{profilePart?.postalAddress?.String || <span className="italic text-gray-400">Not Set</span>}</dd>
                        </div>
                        {userPart?.userType === 'agent' && (
                            <div>
                                <dt className="font-medium text-gray-500">Agency Name</dt>
                                <dd className="text-gray-800 mt-1">{profilePart?.agencyName?.String || <span className="italic text-gray-400">Not Set / Not Applicable</span>}</dd>
                            </div>
                        )}
                        <div>
                            <dt className="font-medium text-gray-500">PAN</dt>
                            <dd className="text-gray-800 mt-1">{profilePart?.pan?.String || <span className="italic text-gray-400">Not Set</span>}</dd>
                        </div>
                    </div>
                </dl>
            </div>
            ...
            <div className="bg-white rounded-lg shadow p-6 md:p-8 border border-gray-200 mb-6">
                ...
                <dl className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-4 text-sm">
                    <div>
                        ...
                        <dd className="text-gray-800 mt-1">{profilePart?.bankName?.String || <span className="italic text-gray-400">Not Set</span>}</dd>
                    </div>
                    <div>
                        <dt className="font-medium text-gray-500">Account Number</dt>
                        <dd className="text-gray-800 mt-1">{profilePart?.bankAccountNo?.String || <span className="italic text-gray-400">Not Set</span>}</dd>
                    </div>
                    <div>
                        <dt className="font-medium text-gray-500">IFSC Code</dt>
                        <dd className="text-gray-800 mt-1">{profilePart?.bankIfsc?.String || <span className="italic text-gray-400">Not Set</span>}</dd>
                    </div>
                </dl>
            </div>
            ...
            <div className="bg-white rounded-lg shadow p-6 md:p-8 border border-gray-200 mb-6">
                <div className="flex justify-between items-center mb-4 pb-3 border-b">
                    ...
                    <Button onClick={openEditPocModal} variant="outlineSm" className="text-xs">
                        <i className="fas fa-pencil-alt mr-2"></i>Edit Contacts
                    </Button>
                </div>
                {insurerPOCs.length === 0 ? (
                    <p className="text-sm text-gray-500 italic">No insurer contacts saved. Click 'Edit Contacts' to add.</p>
                ) : (
                    <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3 text-sm">
                        {insurerPOCs.map((poc, index) => (
                            <div key={poc.id || index}>
                                <dt className="font-medium text-gray-500">{poc.insurerName}</dt>
                                <dd className="text-gray-800 mt-1 break-words">{poc.pocEmail}</dd>
                            </div>
                        ))}
                    </dl>
                )}
            ...
            <EditAgentProfileModal
                ...
                currentProfileData={profilePart}
            />
            <EditAgentGoalModal
